"""
Unified PAIME Application Entry Point
Combines FastAPI (Analytics API) and Dash (Dashboard) into a single application
Designed for Railway deployment with single port exposure
"""
...
async def lifespan(app: FastAPI):
    ...
    # Start Dash dashboard in separate thread
    def run_dash():
        dash_app.run_server(
            debug=False,
            host='0.0.0.0',
...
app = FastAPI(
    ...
    **Endpoints:**
    - API: /analytics/* (RESTful API endpoints)
...
async def root():
    ...
    <html>
    <head>
        <title>PAIME - PlayMetric AI Metrics Engine</title>
        <style>
            body {
    ...
    <body>
        <div class="container">
            <h1>🎮 PAIME</h1>
            <div class="subtitle">PlayMetric AI Metrics Engine</div>
            <p>AI-Powered Game Analytics Platform with Real-time Insights</p>
            
            <div class="cards">
                <a href="/dashboard" class="card">
                    <div class="card-icon">📊</div>
                    <div class="card-title">Dashboard</div>
                    <div class="card-desc">Interactive analytics dashboard with real-time metrics</div>
                </a>
                
                <a href="/docs" class="card">
                    <div class="card-icon">📚</div>
                    <div class="card-title">API Docs</div>
                    <div class="card-desc">RESTful API documentation (Swagger UI)</div>
                </a>
                
                <a href="/analytics/overview" class="card">
                    <div class="card-icon">📈</div>
                    <div class="card-title">Analytics API</div>
                    <div class="card-desc">JSON analytics endpoints for integration</div>
                </a>
                
                <a href="/health" class="card">
                    <div class="card-icon">💚</div>
                    <div class="card-title">Health Check</div>
                    <div class="card-desc">System status and service health</div>
                </a>
            </div>
            
            <div class="footer">
                PAIME v1.0 | Built with FastAPI + Dash + AI/ML
            </div>
        </div>
    </body>
    </html>
...
async def get_user_analysis(user_id: str):
    ...
    try:
        ...
        max_level = max([e.get('levelId', 0) for e in level_events], default=0)
        
        # Spending
        total_spent = sum([
            e.get('priceInRealMoney', 0) 
            for e in user_events['economy_events']
            if e.get('priceInRealMoney')
        ])
